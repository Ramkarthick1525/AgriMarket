...
const ProductCard = ({ product }) => {
  ...
  const handleRentNow = (e) => {
    ...
    // Save rental request in localStorage or show a modal (optional)
    const rentKey = `rent_requests_${user.email}`;
    const existingRequests = JSON.parse(localStorage.getItem(rentKey)) || [];

    const alreadyRequested = existingRequests.some(item => item.id === product.id);
    if (alreadyRequested) {
      toast.error('You already requested this machinery');
      return;
    }

    const updatedRequests = [...existingRequests, product];
    localStorage.setItem(rentKey, JSON.stringify(updatedRequests));
